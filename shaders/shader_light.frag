...
vec4 CalcLightByDirection(Light light, vec3 directionToLight)
{
...
{
	vec3 directionToLight = pLight.position - FragPos;
	float distance = length(directionToLight);
	directionToLight = normalize(directionToLight);

	vec4 lightColor = CalcLightByDirection(pLight.base, directionToLight);
...
vec4 CalcSpotLight(SpotLight sLight)
{
	...
	if(slFactor > sLight.edge)
	{
		...
		return lightColor * (1.0f - (1.0f - slFactor) * (1.0f / (1.0f - sLight.edge)));
	}
	else
	{
